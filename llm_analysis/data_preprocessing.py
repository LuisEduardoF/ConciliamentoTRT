...
if __name__ == '__main__':
    # df = load_dataset("data/updated_dataset.parquet.gzip")
    
    # df.to_parquet("data/updated_dataset_preprocessed.parquet.gzip", compression='gzip')
    # print("Saved preprocessed dataset.")
    
    df= pd.read_parquet("data/updated_dataset_preprocessed.parquet.gzip")
    
    window_df = create_rolling_windows(df, date_col=TIME_COL, window_size=2, window_step=6)
    
    for window_index, window in enumerate(window_df):
        # Create window directory
        window_dir = f'data/window_data/window_{window_index}'
        os.makedirs(window_dir, exist_ok=True)
        
        window.to_parquet(f"data/window_data/window_{window_index}/window_{window_index}.parquet.gzip", compression='gzip')
        print(f"Saved window {window_index} with {len(window)} rows.")

        train_df, test_df = train_test_split(window, test_size=0.3, random_state=42, shuffle=True)
        
        train_df.to_parquet(f"data/window_data/window_{window_index}/train_window_{window_index}.parquet.gzip", compression='gzip')
        test_df.to_parquet(f"data/window_data/window_{window_index}/test_window_{window_index}.parquet.gzip", compression='gzip')
        print(f"Saved train/test split for window {window_index}.")
